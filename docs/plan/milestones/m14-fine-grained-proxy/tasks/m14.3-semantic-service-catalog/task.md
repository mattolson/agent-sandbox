...
without quietly broadening the host allowlist. `m17` remains about the higher-level GitHub API access surface;
adding Git smart-HTTP to `m14.3` does not replace that milestone.
...
- If future services duplicate the pattern of "named surfaces plus scoped selectors," revisit whether the per-service
  ...
  rich service exists.
- Downstream `m17` still owns the GitHub API access work; the repo-scoped `api` surface added here intentionally
