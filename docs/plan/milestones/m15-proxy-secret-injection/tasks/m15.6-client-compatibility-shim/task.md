...
- Resolved for this plan: m15.6 does not implement provider API-key auth, GitHub REST auth, or stock `gh` support. M18
  owns provider API-key injection, M17 owns GitHub API access via `gh api`, and M20 owns residual real
  credential-helper delivery.
