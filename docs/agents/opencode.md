...
Run OpenCode in a network-locked container. All outbound traffic is routed through an enforcing proxy that blocks requests to domains not on the allowlist.
...
Because the sandbox proxy only allows traffic to domains on the allowlist, you must add the appropriate provider service to your network policy. For example, to use OpenCode with Anthropic:
